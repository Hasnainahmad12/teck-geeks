...
import DeleteOperatingSystem from "../components/OperatingSystem/DeleteOperatingSystem";

function OperatingSystem() {
  const { data, error, isLoading } = useGetOperatingSystemQuery();
  ...
  return (
    <SideNav>
            ...
            <div className>
              ...
              <div className="flex flex-wrap items-center justify-end px-4 py-2 border-b">
                <div className="flex px-4 py-2 mb-4 border border-gray-300 rounded-md md:mb-0">
                  <Link
                    to={"/operating-system/add"}
                    className="flex items-center text-gray-700 hover:text-blue-600"
                  >
                    <IoAddOutline />
                    <span className="mr-2 text-xs ">Add Operating System</span>
                  </Link>
                </div>
              </div>
                  ...
                  {data?.data?.map((plan, index) => (
                    <tr key={index} className="border-b border-gray-200">
                      ...
                      <td className="px-6 text-sm font-medium flex">
                        <Link
                          to={`/operating-system/edit/${plan._id}`}
      ...
      {isDelete && (
        <DeleteOperatingSystem onClose={() => setIsDelete(false)} id={deleteId} />
      )}
    </SideNav>
  );
}
