...
import DeleteAsin from "../components/Asin/DeleteAsin";

function Asin() {
  const { data, error, isLoading } = useGetAsinQuery();
  ...
  return (
    <SideNav>
            ...
            <div className>
              ...
              <div className="flex flex-wrap items-center justify-end px-4 py-2 border-b">
                <div className="flex px-4 py-2 mb-4 border border-gray-300 rounded-md md:mb-0">
                  ...
                  >
                    <IoAddOutline />
                    <span className="mr-2 text-xs ">Add Asin</span>
                  </Link>
                </div>
              </div>
              <table className="w-full table-auto">
                <thead className="bg-gray-100">
                  <tr className="text-xs text-left text-gray-500 border-b border-gray-200">
                    <th className="px-6 py-3 font-medium">Asin Name</th>
      ...
      {isDelete && (
        <DeleteAsin onClose={() => setIsDelete(false)} id={deleteId} />
      )}
    </SideNav>
  );
}
