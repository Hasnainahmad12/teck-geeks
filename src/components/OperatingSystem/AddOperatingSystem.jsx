...
// api
import { useAddOperatingSystemMutation } from "../../apis/OperatingSystem/OperatingSystem";
...
function AddOperatingSystem() {
  ...
  const handleAddPackage = async (e) => {
    e.preventDefault();
    const operatingname = e.target[0].value;
  ...
  useEffect(() => {
    if (isSuccess) {
      toast.success("Operating System added successfully");
      navigate("/operating-system");
    } else if (isError) {
      toast.error(error?.data?.error || "Failed to add Operating System");
    }
  }, [isSuccess, isError]);
