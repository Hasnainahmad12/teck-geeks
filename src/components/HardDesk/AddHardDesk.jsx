...
// api
import { useAddHardDeskMutation } from "../../apis/AddHardDesk/AddHardDesk";

function AddHardDesk() {
  ...
  const handleAddPackage = async (e) => {
    e.preventDefault();
    const hardiskname = e.target[0].value;
    const status = e.target[1].value;
    
    const data = {
      hardiskname: hardiskname,
      status: status,
    };
    await PostPlan(data);
  };
  ...
  useEffect(() => {
    if (isSuccess) {
      toast.success("Hardisk added successfully");
      navigate("/hard-desk");
    } else if (isError) {
      toast.error(error?.data?.error || "Failed to add Hardisk");
    }
  }, [isSuccess, isError]);
