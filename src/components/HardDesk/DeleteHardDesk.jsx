...
import { useDeleteHardDeskMutation } from "../../apis/AddHardDesk/AddHardDesk";

function DeleteHardDesk({ onClose, id }) {
  const [deletePlan, { isLoading, isSuccess, isError, error }] =
  useDeleteHardDeskMutation();

  useEffect(() => {
    if (isSuccess) {
      toast.success("Hardisk deleted successfully");
      onClose();
    } else if (isError) {
      toast.error(error?.data?.error || "Failed to delete Hardisk");
    }
          ...
          <div className="relative mx-4 overflow-hidden text-left transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:max-w-lg sm:w-full">
            <div className="m-10 flex max-w-lg flex-col items-center px-8 py-10 text-gray-800 ">
              ...
              <p className="mt-2 text-center text-lg">
                Are you sure you want to delete the Hard Desk
