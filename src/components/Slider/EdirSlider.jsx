...
import { useUpdateSliderMutation } from "../../apis/Slider/Slider";
...
function EditSlider() {
  ...
  useEffect(() => {
    const handleUpdateData = async () => {
      try {
        const res = await axios.get(`https://fierce-veil-elk.cyclic.app/slider/${id}`);
          console.log(res?.data);
          setStatus(res?.data?.data?.status.toString() || "");
          setPreview(res?.data?.data?.image);
        } 
        catch (error) {
          toast.error(error?.data?.error || "Error fetching Slider data");
        }
    };
